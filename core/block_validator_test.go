...
func TestStateRootValidationGate(t *testing.T) {
	tests := []struct {
		name             string
		blockTime        uint64
		jadeForkTime     *uint64
		...
		{
			name:             "before Jade fork",
			blockTime:        500,
			jadeForkTime:     uint64Ptr(1000),
			expectValidation: false,
			description:      "pre-Jade block carries legacy zkTrie root — skip validation",
		},
		{
			name:             "at Jade fork",
			blockTime:        1000,
			...
			description:      "block at Jade fork boundary — validate MPT root",
		},
		{
			name:             "after Jade fork",
			blockTime:        1500,
			jadeForkTime:     uint64Ptr(1000),
			expectValidation: true,
			description:      "post-Jade block is native MPT — validate",
		},
		{
			...
			jadeForkTime:     nil,
			expectValidation: false,
			description:      "JadeForkTime not set — validation skipped",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			shouldValidate := config.IsJadeFork(tt.blockTime)
