...
import (
	...
	lru2 "github.com/morph-l2/go-ethereum/common/lru"
...
type Database interface {
	...
	OpenStorageTrie(addrHash, root, origin common.Hash) (Trie, error)
...
func (db *cachingDB) OpenTrie(root common.Hash) (Trie, error) {
	if db.morphZktrie {
		tr, err := trie.NewMorphZkTrie(root, root, db.db, rawdb.TrieNodeAccountPrefix)
		if err != nil {
			return nil, err
		}
		return tr, nil
	...
	if db.zktrie {
		tr, err := trie.NewZkTrie(root, db.db)
		if err != nil {
			return nil, err
		}
		return tr, nil
...
func (db *cachingDB) OpenStorageTrie(addrHash, root, origin common.Hash) (Trie, error) {
	if db.morphZktrie {
		...
		tr, err := trie.NewMorphZkTrie(root, origin, db.db, prefix)
		if err != nil {
			return nil, err
		}
		return tr, nil
