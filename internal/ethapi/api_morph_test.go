...
func TestDoEstimateGasCapsAtMaxTxGasPostAmsterdam(t *testing.T) {
	...
	_, err := DoEstimateGas(context.Background(), backend, TransactionArgs{
		To:   &to,
		Data: &data,
	}, rpc.BlockNumberOrHashWithNumber(rpc.LatestBlockNumber), nil, 0)
	if err == nil {
...
func TestAccessListAuthorizationLimitUsesAmsterdamPricing(t *testing.T) {
	...
	t.Run("pre-Amsterdam rejects conservatively", func(t *testing.T) {
		...
		_, _, _, err := AccessList(context.Background(), backend, rpc.BlockNumberOrHashWithNumber(rpc.LatestBlockNumber), makeArgs(), nil)
		if err == nil {
	...
	t.Run("post-Amsterdam accepts same gas budget", func(t *testing.T) {
		...
		acl, gasUsed, vmErr, err := AccessList(context.Background(), backend, rpc.BlockNumberOrHashWithNumber(rpc.LatestBlockNumber), makeArgs(), nil)
		if err != nil {
...
func (m *estimateGasBackend) HeaderByNumberOrHash(context.Context, rpc.BlockNumberOrHash) (*types.Header, error) {
	return types.CopyHeader(m.header), nil
}
