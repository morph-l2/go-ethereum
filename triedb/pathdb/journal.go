...
func newJournalWriter(file string, db ethdb.KeyValueStore, journalType JournalType) JournalWriter {
...
func newJournalReader(file string, db ethdb.KeyValueStore, journalType JournalType) (JournalReader, error) {
	if journalType == JournalKVType {
...
func (db *Database) loadLayers() layer {
	...
	_, root := rawdb.ReadAccountTrieNode(db.diskdb, zkt.TrieRootPathKey[:])
...
func (db *Database) loadDiskLayer(r *rlp.Stream, journalTypeForReader JournalType) (layer, error) {
	...
	for _, entry := range encoded {
		nodes.Put(entry.Path, entry.Blob)
	}
...
func (dl *diskLayer) journal(w io.Writer, journalType JournalType) error {
	...
	nodes := make([]journalNode, 0, len(bufferNodes))
	for _, v := range bufferNodes {
		entry := journalNode{Path: v.K, Blob: v.V}
